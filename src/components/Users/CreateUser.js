...
const CreateUser = (props) => {
    ...
    const createUserHandler = (event) => {
        ...
        if(inputName.trim().length === 0 || inputAge.trim().length === 0) { //Проверяем длену данных, что бы не были пусты поля
            setError({
        ...
        if(+inputAge < 1) { //+ в начаеле приоброзует в чесло. Проверяем что бы возрост не был меньше 1
            setError({
